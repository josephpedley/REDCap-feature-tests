describe('Assign administrators and account managers', () => {
    it('Assigns administrators and account managers', () => {
         cy.loginAdmin();
         cy.get('a[href="/redcap_v15.4.2/ControlCenter/index.php"]').should('be.visible').click();
         cy.get('a[href="/redcap_v15.4.2/ControlCenter/superusers.php"]').click()
         ...
         cy.get('#user_search').type('test_Joe')
        ...
        .should('be.visible')
         ...
         cy.loginTestUser();
         cy.get('a[href="/redcap_v15.4.2/ControlCenter/index.php"]').click();
         cy.get('a[href="/redcap_v15.4.2/ControlCenter/superusers.php"]').click()
         cy.contains('Set administrator privileges').should('be.visible')
         cy.contains('Administrators').should('be.visible')
         ...
         cy.contains('Set administrator privileges').should('be.visible')
         cy.get('#80-super_user').should('exist').uncheck({force: true})
         cy.get('body').click(0, 0)
         cy.get('#80-account_manager').uncheck({force: true})
         cy.get('#80-access_system_config').uncheck({force: true})
         cy.get('#80-access_system_upgrade').uncheck({force: true})
         cy.get('#80-access_external_module_install').uncheck({force: true})
         cy.get('#80-access_admin_dashboards').uncheck({force: true})
         ...
         cy.contains('a', 'Administrator Privileges').should('be.visible')
         cy.contains('a', 'Browse Project').should('be.visible')
         cy.contains('a', 'General Configuration').should('not.be.visible')
         cy.contains('a', 'Edit Project Settings').should('not.be.visible')
         cy.contains('a', 'Add Users').should('be.visible')
